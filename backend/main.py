...
async def create_pr(request: PRRequest):
    ...
    async with httpx.AsyncClient() as client:
        ...
        ref_response = await client.get(
            f"https://api.github.com/repos/{request.owner}/{request.repo}/git/ref/heads/{default_branch}",
            headers=headers,
        )
        ...
        if not base_sha:
            return {"error": "Could not find base branch", "github_response": ref_data}

        # Step 2: Create a new branch pointing at that same commit
        branch_name = f"devpilot/{uuid.uuid4().hex[:8]}"
        create_branch_response = await client.post(
            f"https://api.github.com/repos/{request.owner}/{request.repo}/git/refs",
            headers=headers,
        ...
        # Step 3: Commit each changed file to the new branch
        for f in request.files:
            # Check if the file already exists (need its SHA to update it)
            existing_response = await client.get(
                f"https://api.github.com/repos/{request.owner}/{request.repo}/contents/{f.path}",
                headers=headers,
                params={"ref": branch_name},
            )
            existing_data = existing_response.json()
            existing_sha = existing_data.get("sha") if existing_response.status_code == 200 else None

            content_encoded = base64.b64encode(f.new_content.encode("utf-8")).decode("utf-8")

            commit_payload = {
                "message": f"DevPilot: {request.task}",
            ...
            if commit_response.status_code not in (200, 201):
                return {"error": f"Failed to commit {f.path}", "details": commit_response.json()}
        ...
        pr_response = await client.post(
            ...
            headers=headers,
            json={
                "title": f"DevPilot: {request.task}",
                "head": branch_name,
                "base": default_branch,
                "body": f"**Task:** {request.task}\n\n**Summary:** {request.summary}\n\n_Generated by DevPilot AI agents._",
            },
        )
        pr_data = pr_response.json()

        if pr_response.status_code not in (200, 201):
            return {"error": "Failed to create pull request", "details": pr_data}
